...
1. **売上ランキングタブ廃止**
   - 売上ランキングタブとその全機能を削除
   - 月遷移時のデータ更新問題も解決（機能自体を削除）
   - 関連する状態変数、関数、UIコンポーネントをすべて除去
   - タブ切り替え不要によりナビゲーションUIも簡素化

2. **店舗管理テーブル表示最適化**
   - **削除項目**: 店舗ID、作成日（非表示に変更）
   - **名称変更**: 今月売上→売上(税込)、今月客数→客数、平均客単価→客単価
   - **維持項目**: 店舗名、店舗コード、操作ボタン
   - レスポンシブ対応と視認性向上

3. **CSV出力機能実装**
   - 出力フィールド: 店舗名、店舗コード、売上(税込)、客数、客単価
   - ファイル名: `店舗売上データ_YYYY年MM月.csv`
   - UTF-8 BOM付きで日本語文字化け対策
   - 管理画面ヘッダーにCSV出力ボタン追加

### 削除済み機能と置き換え
- **売上ランキング**: 廃止（店舗管理画面のCSV出力で代替）
- **月別売上データAPI**: 使用停止
- **ランキング用状態管理**: 完全削除
- **複雑なタブナビゲーション**: 単一画面に統一

### スケーラビリティ検討（FC本部本格運用向け）
1. **現在の無料版構成での制約（300店舗規模）**
   - Supabase: DB容量500MB制限（2年分データで超過確実）
   - Vercel: 月100GB帯域幅制限（300店舗で超過可能性）
   - GitHub: 問題なし

2. **推奨課金計画**
   - **当面（150店舗）**: Supabase Pro $25/月のみ
   - **本格運用（300店舗）**: Supabase Pro + Vercel Pro = $45/月
   - **法人移行時**: 全サービス法人アカウント = $80/月
...
- 管理画面は単一画面構成を維持（タブ切り替え廃止）
- CSV出力機能のフィールド構成は変更時要注意